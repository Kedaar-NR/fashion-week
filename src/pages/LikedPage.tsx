...
const LikedPage = () => {
  ...
  const filteredBrands = brands.filter(brand => likedBrands.includes(brand.name));
  ...
  return <div className="flex min-h-screen font-kanit bg-zinc-950">
      ...
      <div className="flex-1 ml-14 md:ml-48 p-8">
        ...
        {!isLoggedIn && <div className="p-6 bg-gray-50 rounded-lg border border-gray-100 text-center max-w-2xl mx-auto">
            ...
            <a href="/signin" className="inline-flex items-center px-4 py-2 bg-black text-white rounded-md hover:bg-gray-800 transition-colors">
              Sign In
            </a>
          </div>}
        
        {isLoggedIn && filteredBrands.length === 0 && <div className="p-6 bg-gray-50 rounded-lg border border-gray-100 text-center max-w-2xl mx-auto">
            ...
            <a href="/home" className="inline-flex items-center px-4 py-2 bg-black text-white rounded-md hover:bg-gray-800 transition-colors">
              Browse Brands
            </a>
          </div>}
        
        {isLoggedIn && filteredBrands.length > 0 && !selectedBrand && <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {filteredBrands.map(brand => <div key={brand.name} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow p-4 flex flex-col items-center cursor-pointer" onClick={() => handleBrandClick(brand.name)}>
        ...
        {selectedBrand && <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center ml-14 md:ml-48">
            <div className="bg-white rounded-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-auto">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => handleUnlike(selectedBrand)} className="p-2 hover:bg-gray-100 rounded-full">
              ...
              <div className="rounded-xl overflow-hidden aspect-square w-full h-[70vh]">
                <iframe src={`https://www.instagram.com/${selectedBrand}/embed`} className="w-full h-full border-none" title={`${selectedBrand} Instagram Feed`} allowTransparency={true} scrolling="no" />
              </div>
            </div>
          </div>}
      </div>
