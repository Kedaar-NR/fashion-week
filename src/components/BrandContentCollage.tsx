          ...
          <div className="grid grid-cols-3 gap-4">
            {[...selectedContent.videos, ...selectedContent.images].map(
              (content, index, array) => {
                ...
                // Center items in last row if it's incomplete
                let colStartClass = "";
                if (isInLastRow && itemsInLastRow < 3) {
                  const offset = itemsInLastRow === 1 ? 2 : 1; // Start at col 2 for single item, col 1 for two items
                  if (itemsInLastRow === 1) {
                    colStartClass = "col-start-2";
                  } else if (itemsInLastRow === 2 && index === totalItems - 2) {
                    colStartClass = "col-start-1 col-end-3";
                  }
                }
                ...
                return (
                  <motion.div
                    key={`content-${index}`}
                    className={`overflow-hidden rounded-lg shadow-md w-full ${colStartClass}`}
