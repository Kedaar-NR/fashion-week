...
const MarqueeGenres = ({
  ...
  return <div className="w-full overflow-hidden text-white py-4 bg-zinc-50">
      ...
      ease: "linear"
        ...
        {duplicatedGenres.map((genre, idx) => <button key={`${genre}-${idx}`} onClick={() => onSelectCategory(genre)} className={`mx-8 px-6 py-2 text-lg font-bold uppercase ${genreColors[genre] || 'bg-gray-500'} rounded-full hover:opacity-90 transition-opacity shadow-lg`}>
            {genre}
          </button>)}
      </motion.div>
    </div>;
